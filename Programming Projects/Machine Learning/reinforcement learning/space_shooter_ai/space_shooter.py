...
class Game:
    ...
    def ship_mov(self):
        ...
        if key_pressed[pygame.K_d] and self.ship_pos.x + VEL + self.ship_pos.width < BORDER.x:
            self.ship_pos.x += VEL
        if key_pressed[pygame.K_w] and self.ship_pos.y > 50:
            self.ship_pos.y -= VEL
        if key_pressed[pygame.K_s] and self.ship_pos.y + VEL + self.ship_pos.height < BORDER.y - 50:
            self.ship_pos.y += VEL

    def handle_astroid_beam(self):
        asteroid_index, beam_index = 0, 0
        is_astroid_destroyed = False
    ...
    def get_info(self):
        ...
        height = 200
        # BACK TWO
        self.lines.append(pygame.Rect(self.ship_pos.x, self.ship_pos.y + self.ship_pos.height,
                                      width // 2, height))

        self.lines.append(pygame.Rect(self.ship_pos.x, self.ship_pos.y - height,
                                      width // 2, height))

        # FRONT TWO
        self.lines.append(pygame.Rect(self.ship_pos.x + width // 2, self.ship_pos.y + self.ship_pos.height,
                                      width // 2, height))

        self.lines.append(pygame.Rect(self.ship_pos.x + width // 2, self.ship_pos.y - height,
                                      width // 2, height))

        self.lines.append(pygame.Rect(self.ship_pos.x, self.ship_pos.y, 800, self.ship_pos.height))

        self.lines.append(
            pygame.Rect(self.ship_pos.x + self.ship_pos.width, self.ship_pos.y + self.ship_pos.height // 2, 900, 5))

        self.info = [False] * (len(self.lines) + 1)

        for index in range(len(self.lines)):
            for astroid in self.asteroids:
                if astroid.colliderect(self.lines[index]):
                    self.info[index] = True

        # if info[3] and

        self.info[-1] = (len(self.bullets) != MAX_BULLETS)
        ...
        #         if ast.colliderect(line):
        #             if abs(ast.x // ast.y) == abs(width // height):
        #                 print("COLLLLL")
        # for astroid in self.asteroids:
        #     print([astroid.colliderect(line) for line in self.lines])
        return self.info
    ...
    def play_ai_step(self, action):
        ...
        if self.info[3] and action[2] and self.info[4]:
            print("REWARD")
            reward += 100
        if self.collision:
            reward -= 200
        if self.info[0] or self.info[1]:
            reward -= 10
        elif self.info[2] or self.info[3]:
            reward +=5

        if action[0] and self.ship_pos.y > 50:
            self.ship_pos.y -= VEL
        if action[1] and self.ship_pos.y + VEL + self.ship_pos.height < BORDER.y - 50:
            self.ship_pos.y += VEL

        self.drawWindow()
        return reward, self.collision, int(self.score)
